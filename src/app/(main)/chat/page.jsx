    ...
    <>
      <div className="fixed bottom-4 right-4 z-50">
        <button
          ...
          className="bg-[#5AAA7C] hover:bg-[#46996a] text-white py-3 px-5 rounded-full shadow-xl flex items-center gap-2 transition-all duration-300 hover:scale-105"
        >
          ...
          <div className="bg-white flex flex-col h-[500px] border border-gray-200 rounded-lg">
            ...
            <div ref={chatboxRef} className="flex-1 p-4 overflow-y-auto space-y-3 bg-[#f9fdfa]">
              {messages.map((msg, idx) => (
                <div key={idx} className={msg.type === "user" ? "text-right" : "text-left"}>
                  <p className={`${msg.type === "user" ? "bg-[#5AAA7C] text-white" : "bg-gray-200 text-gray-800"} px-4 py-2 rounded-2xl inline-block`}>{msg.text}</p>
                </div>
              ))}
            </div>
            <div className="border-t p-3 flex gap-2 bg-white">
              <input
                ...
                className="flex-1 px-4 py-2 rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#5AAA7C]"
              />
              <button
                onClick={handleSend}
                className="bg-[#5AAA7C] hover:bg-[#46996a] text-white px-5 py-2 rounded-full shadow-md transition hover:scale-105"
              >
                Send
              </button>
            </div>
