...
const [unreadCount, setUnreadCount] = useState(3); // You can update this dynamically later
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      <h2 className="text-2xl sm:text-3xl mt-6 font-bold text-[#88B29A] mb-10 text-center sm:text-left">
        Appointment
      </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {cards.map((card, index) => (
          ...
          >
            <div className="bg-[#88B29A] h-36 rounded-t-xl w-full flex justify-center items-center relative">
              {card.title === "Chat" && unreadCount > 0 && (
  <div className="absolute top-2 right-2 bg-white text-[#88B29A] text-xs font-bold px-2.5 py-1.5 rounded-full shadow-md animate-pulse">
    {unreadCount}
  </div>
)}
                      ...
                      {card.title === "Meetings" ? (
                        ...
                        <Link href={`/documents/${id}`}>
                          <button className="w-full text-left px-2 py-2 hover:bg-[#f0fdf4] flex items-center gap-3 transition-all duration-200 rounded-md">
                            <span className="bg-[#88B29A] text-white p-1 rounded-full">
                              <Eye
                                size={16}
...
{/* ... */}
            <div className="pt-16 pb-6 px-4 sm:px-6 text-center w-full">
              {card.title === "Upload Document" ? (
                <button
                  ...
                  className="mt-6 px-6 py-2.5 rounded-full bg-[#88B29A] text-white font-semibold shadow-md hover:scale-105 transition"
                >
                  {card.title}
                </button>
              ) : card.title === "Meetings" ? (
                ...
                >
                  <button className="mt-6 px-6 py-2.5 rounded-full bg-[#88B29A] text-white font-semibold shadow-md hover:scale-105 transition">
                    {card.title}
                  </button>
                </Link>
              ) : (
            ...
            {showChat && (
              <div className="fixed my-2  inset-x-0 bottom-0 md:inset-auto md:bottom-8 md:right-6 md:top-22  z-[100] w-full max-w-sm mx-auto md:mx-0 md:max-w-[358px] rounded-2xl shadow-2xl bg-gray-200 animate-slide-up border border-gray-300 overflow-hidden">
                <div className="bg-gradient-to-r from-[#88B29A] to-[#88B29A] text-white px-5 py-4 flex justify-between items-center">
                ...
                <div className="border-t flex items-center bg-white px-3 py-2">
                  <input
                    ...
                    className="w-full px-4 py-2 rounded-l-full text-sm border border-gray-300 focus:ring-2 focus:ring-[#88B29A] outline-none transition"
                  />
                  <button
                    onClick={handleSend}
                    className="bg-[#88B29A] hover:bg-[#88B29A] text-white px-4 py-2 rounded-r-full font-medium hover:scale-105 transition"
                  >
                    <SendHorizonal />
                  </button>
                </div>
    ...
    <div className="w-full max-w-2xl mx-auto mt-4 sm:mt-6 md:mt-10 bg-white rounded-xl shadow-xl px-4 py-6 sm:px-6 md:px-8 space-y-4 sm:space-y-6">
      ...
      <div className="grid gap-4">
        <input
          ...
          className="w-full border border-gray-300 rounded px-4 py-2 sm:py-3 focus:outline-none focus:ring-2 focus:ring-[#88B29A] focus:border-[#88B29A] text-sm sm:text-base"
          value={title}
      ...
      <div
        className={`border-2 border-dashed rounded-xl p-4 sm:p-6 md:p-10 text-center transition ${
          dragOver ? "border-[#88B29A] bg-green-50" : "border-gray-300"
        }`}
        onDrop={handleDrop}
        onDragOver={(e) => {
          e.preventDefault();
      ...
      >
        <div className="flex flex-col items-center space-y-3">
          <Upload className="text-[#88B29A] w-8 h-8 sm:w-10 sm:h-10" />
          <p className="text-sm sm:text-base md:text-lg font-semibold text-gray-700 text-center">
            Drag & drop files or{" "}
            <span
              className="text-[#88B29A] underline cursor-pointer"
      ...
      <button
        ...
        disabled={uploading}
        className="w-full mt-4 bg-[#88B29A] hover:bg-blue -600 text-white font-semibold py-3 rounded-lg transition-all duration-300 text-sm sm:text-base"
      >
      ...
      {uploading && (
        <div className="w-full bg-gray-200 rounded h-3 overflow-hidden mt-4">
          <div
            className="bg-[#88B29A] h-full transition-all"
