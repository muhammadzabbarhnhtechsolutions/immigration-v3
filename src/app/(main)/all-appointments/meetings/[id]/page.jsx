    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      {loading ? (
        <div className="mx-auto bg-[#ebf0ed] px-4 mt-8 py-20 text-center flex flex-col items-center justify-center gap-4 animate-fade-in">
          ...
          {/* Text */}
          <p className="text-[#5bb180] text-lg font-medium">Loading ...</p>
        </div>
      ) : meetings.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 gap-8">
          {meetings.map((meeting, index) => (
            <Link href={meeting.link} key={meeting.id || index} target="_blank">
              <div className="bg-[#5AAA7C] cursor-pointer shadow-xl relative hover:scale-105 transition-transform duration-300 rounded-2xl px-6 py-8 text-white text-center">
