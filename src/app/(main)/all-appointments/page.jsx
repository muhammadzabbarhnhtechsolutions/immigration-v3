...
const AppointmentCard = ({ appointment }) => {
  ...
  return (
    <div className="relative bg-gradient-to-br from-[#5AAA7C]/90 to-[#367f5d]/90 rounded-3xl px-6 py-12 w-full sm:w-[363px] text-white text-center shadow-2xl hover:scale-105 transition-transform duration-300">
      <Link className="flex" href={`/all-appointments/data?appointment_id=${appointment.appointment_id}`}>
        {/* Profile Image */}
        <div className="absolute -top-14 left-1/2 transform -translate-x-1/2">
          <img
            src={appointment.profile || "https://randomuser.me/api/portraits/men/75.jpg"}
            alt="Profile"
        ...
        {/* Text Content */}
        <div className="pt-8  ml-4 space-y-1">
          <h3 className="font-bold text-xl -ml-8 tracking-wide">{appointment.name}</h3>
          <p className="text-sm text-white/90">Email: {appointment.email}</p>
        </div>
      </Link>
    </div>
  );
