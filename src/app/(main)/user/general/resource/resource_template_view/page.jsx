...
function ResourceTemplateViewPage() {
  ...
  // API call to fetch resources based on course_id and category
  useEffect(() => {
    const fetchResources = async () => {
      try {
        ...
        if (courseId && category) {
          ...
          url = `/user/general/resource/resource_template_view/?course_id=${courseId}`;
        } else if (category) {
          url = `/user/general/resource/resource_template_view/?category=${category}`;
        } else {
          url = `/user/general/resource/resource_template_view/`;
        }
  ...
  const filteredResources = resources.filter((r) => {
    ...
    return (
      ...
      r.documents?.toLowerCase().includes(q)
    );
  });
    ...
    <section className="px-4 sm:px-12 py-16">
      ...
      <div className="flex flex-col sm:flex-row items-start md:mt-6 sm:items-center justify-between gap-4 mb-10">
        <div className="flex items-center gap-4">
          <button
            onClick={() => router.back()}
            className="p-2 rounded-full bg-white border border-[#88B29A] text-[#88B29A] hover:bg-[#88B29A] hover:text-white transition-all duration-300 shadow-sm"
          >
