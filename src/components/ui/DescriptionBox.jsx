...
export default function DescriptionBox({ videoid }) {
  ...
  const AddUpdateTask = async (taskId) => {
    ...
    try {
      const result = await TaskUpdate({ id: taskId, text: updatedAnswer, file: file });
      if ("data" in result) {
        const Data = result.data;
        if (Data?.status) {
          setIsEdit(false);
    ...
    <div className=' bg-opacity-40 p-4 md:p-8 rounded-xl flex flex-col gap-5 '>
      {apiResponse && apiResponse.length > 0 ? (
            ...
            <div key={task.id} className="mb-8">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div className="flex flex-col">
                  <span className="text-lg font-extrabold text-[#86ffbb]">{task.admin_name}</span>
                  <span className="text-md font-semibold text-dark"> {task.task_title}</span>
                </div>
              </div>
              ...
              <div className="ml-14">
                {task.answers.length > 0 ? (
                  task.answers.map((answer, answerIndex) => (
                    <div key={answerIndex} className='bg-[#FFFFFF9C] p-3 rounded-[11px] mt-5 relative'>
                      {!isEdit ? (
                        <>
        ...
        <div className="flex items-center justify-center h-55">
          <p className="text-gray-500 text-2xl font-semibold">No tasks found</p>
        </div>
      )}
    </div>
  );
