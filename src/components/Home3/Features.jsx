      ...
      <div className="max-w-7xl mt-28 md:mt-14 ">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
          {keyFeatures.map(({ icon, title, text, isSvg }, idx) => (
            <div
              key={idx}
              className=" flex items-center justify-center flex-col cursor-pointer border bg-[#3F855D40] border-gray-200  p-6 py-26 rounded-2xl hover:shadow-xl transition-all duration-300"
            >
              <div className="mb-4 h-[100px] w-[100px] flex items-center text-center justify-center">
                <Image
                  src={icon}
                  alt={title}
                  width={234}
              ...
              <h3 className="font-semibold  mx-12 text-center text-[21px] text-gray-800 mb-2">
                {title}
              </h3>
            </div>
          ))}
